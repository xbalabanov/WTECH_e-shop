    ...
    <main>
      <div class="main-product-section">
        ...
        <div class="product-details">
          ...
          <p class="product-price">
            {{ number_format((float) $book->discounted_price, 2, ',', '.') }}&euro;
            <span class="product-discount-badge">-{{ (int) ($book->discount ?? 0) }}%</span>
          </p>
          <p class="product-description">{{ (int) ($book->stock ?? 0) }} in stock</p>
          <div class="product-actions">
            ...
            @elseif (($cartQty ?? 0) > 0)
              <div class="product-qty" aria-label="Cart quantity controls">
                ...
                @else
                  <button type="button" class="product-qty-btn" aria-label="Maximum stock reached" disabled>+</button>
                @endif
              </div>
            @else
      ...
      <section class="recomended" aria-labelledby="recommended-title">
        ...
        <div class="recomended-cards">
          @forelse ($recommendedBooks as $recBook)
            <article class="card" role="group">
              <a href="{{ route('products.show', $recBook) }}" aria-label="Open {{ $recBook->title }} details">
                <div class="cover" aria-hidden="true" @if ($recBook->cover_image_url) style="background-image: url('{{ $recBook->cover_image_url }}'); background-size: cover; background-position: center;" @endif></div>
              </a>
              <div class="card-elements">
                <div class="card-title">
                  <h3 class="title">{{ $recBook->title }}</h3>
                  <p class="author">{{ $recBook->authors?->pluck('full_name')->join(', ') ?: 'Unknown author' }}</p>
                </div>
                <p class="card-description">
                  {{ substr($recBook->description ?? 'No description available', 0, 100) }}...
                </p>
                <div class="card-info-container">
                  <p class="card-book-type">{{ $recBook->genre ?? 'General' }}</p>
                ...
                <div class="card-actions">
                  <form method="POST" action="{{ route('cart.add') }}" style="width: 100%;">
                    ...
                    <input type="hidden" name="quantity" value="1" />
                    <button class="card-cart-button" type="submit" aria-label="Add {{ $recBook->title }} to cart">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
    ...
    <footer class="site-footer">
      <div class="footer-inner">
        ...
        <div class="footer-bottom">
          <div class="copyright">┬ę 2026 Eunoia. All rights reserved.</div>
